...
function findFirstSingleChar(str) {
  let symb = '';
  const splitted = str.split('').sort();
  splitted.map((char, index) => {
    if (!splitted.slice(index + 1).includes(char) && !splitted.slice(0, index).includes(char)) {
      symb += char;
      return char;
    }
    return char;
...
function isCreditCardNumber(ccn) {
  ...
  const newArr = str.map((num, index) => {
    if (index === 0) {
      return num;
    }
    if ((index + 1) % 2 === 0) {
  ...
  return newArr.reduce((sum, acc) => Number(sum) + Number(acc)) % 10 === 0;
}
...
function getDigitalRoot(num) {
  let result = num.toString().split('').reduce((sum, acc) => Number(sum) + Number(acc));
  if (result > 9) {
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (openBrackets.includes(arr[i])) {
      ...
      if (openBrackets.indexOf(lastOpenBracket) !== closedBrackets.indexOf(arr[i])) {
        return false;
      }
    }
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  const commonPath = [];
  for (let i = 0; i < pathParts[0].length; i += 1) {
